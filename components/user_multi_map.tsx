...
const User_multiMAP = () => {
  ...
  const [selectedUser, setSelectedUser] = useState<number>(1); // Track the selected user
  ...
  useEffect(() => {
    loadGoogleMapsAPI().then(() => {
      ...
      if (data.length > 0) {
        ...
        const filteredData = data.filter((todo) => todo.user === selectedUser); // Filter data by selected user
